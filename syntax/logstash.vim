...
syn region logstashString start=~\"~          end=~\"~     skip=~\v\#\|\\.~  contains=grokCapture  contained 
syn region logstashString start=~\'~          end=~\'~     skip=~\v\#\|\\.~  contains=grokCapture  contained 
...
syn region logstashVariableBlock start=~\[~ end=~\]~ skip=~\v\#\|\\.~ contains=logstashString contained transparent
...
syn keyword logstashOperator and or nand xor    contained
...
syn keyword logstashBoolean     true false      contained
...
hi link grokCapture            SpellCap
hi link grokVariable           SpellLocal
